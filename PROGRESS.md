...
- `backend/app/config.py`: `error_bot_url: str = ""` 추가
- `backend/app/main.py`: error_bot_url 설정 시 ErrorReporterMiddleware 등록
...
- `curl -X POST http://localhost:8001/api/test-webhook` 테스트 에러 전송 검증
